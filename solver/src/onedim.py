...
class FlameBase(Sim1D):
    ...
    def progressing_variable(self):
        """
        Get the progressing variable for FGM method.
        The two forms of progressing variable are:
        form_1:
            Yc = a*Y1 + b*Y2  
            a, b are both unit, and Y1, Y2 are mass fraction of two selected species.
        form_2:
            Yc = (T - Tu)/(Tb - Tu)
            Tu is unburnt gas temperature
            Tb is burnt gas temperature
        """

        val1 = ((self.gas.Y[self.gas.species_index('CO2')])/44
                 +(self.gas.Y[self.gas.species_index('H2O')])/18
                 +(self.gas.Y[self.gas.species_index('H2')])/2) 
        val1_u = ((self.Y[self.gas.species_index('CO2')][0])/44 
                 +(self.Y[self.gas.species_index('H2O')][0])/18
                 +(self.Y[self.gas.species_index('H2')][0])/2)
        val1_b = ((self.Y[self.gas.species_index('CO2')][-1])/44 
                 +(self.Y[self.gas.species_index('H2O')][-1])/18
                 +(self.Y[self.gas.species_index('H2')][-1])/2)
        norm_val1 = (val1 - val1_u)/(val1_b - val1_u)
    
        '''
        Tu = self.inlet.T
        Tb = self.T[-1]
        val2 = (self.gas.T - Tu)/(Tb - Tu)
        ''' 
        return vals
    ...
    def OmegaYc(self):
        ...
        [kmol/m^2/s]
        """
        val1 = (self.gas.net_production_rates[self.gas.species_index('CO2')]
               +self.gas.net_production_rates[self.gas.species_index('H2O')]
               +self.gas.net_production_rates[self.gas.species_index('H2')])
        return val1


    def mixture_fraction(self):
        """
        this case is based on the defination of mixture fraction of 
        m1/m1+m2
        """
        val1 = self.Y[self.gas.species_index('CH4')][0]
        return val1

    def write_csv(self, filename, species='Y', quiet=True):
        """
        ...
        #self = f = ct.FreeFlame()  is an instance of class FlameBase
        writer.writerow(['T (K)'] + self.gas.species_names)
        for n in range(self.flame.n_points):
            self.set_gas_state(n)
            writer.writerow([T[n]] + list(getattr(self.gas, species)))
        csvfile.close()
        if not quiet:
